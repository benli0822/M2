...
(function() {
    ...
    window.test = function(testName, expected, callback) {
        QUnitTestCase.prototype['test ' + testName] = function() {
            ...
            this.div.setAttribute("id","qunit-fixture");
            this.body.appendChild(this.div);
          
            callback.call(this.lifecycle);
    ...
    window.equal = function(result, expected, msg) {
        window.ok(result==expected,msg);
    };

    window.throws = function(callback, exc , msg) {
            var actual;
            try {
                callback.call(QUnitTestCase.prototype.lifecycle);
            } catch (e) {
                actual = e;
            }
            if (actual) {
                assertInstanceOf(exc,actual);
            } else
                fail (exc.name+" exception expected!");
    }
    ...
    window.QUnit_orig=window.QUnit;
